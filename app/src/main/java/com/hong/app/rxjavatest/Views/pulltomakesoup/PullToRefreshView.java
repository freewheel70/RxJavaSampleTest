package com.hong.app.rxjavatest.Views.pulltomakesoup;
...
import com.hong.app.rxjavatest.Views.pulltomakesoup.refresh_view.SoupRefreshView;
import com.hong.app.rxjavatest.Views.pulltomakesoup.utils.Utils;
...
public class PullToRefreshView extends ViewGroup {
    ...
    public PullToRefreshView(Context context, AttributeSet attrs) {
        ...
        final int type = a.getInteger(R.styleable.RefreshView_refreshType, STYLE_SOUP);
        a.recycle();
        ...
        if (getChildCount() > 0) {
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (child != mRefreshView) {
                    ...
                    mTarget.addOnScrollListener(new RecyclerView.OnScrollListener() {
                        ...
                        public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                            mRecyclerViewIsDragging = (newState != RecyclerView.SCROLL_STATE_IDLE);
    ...
    boolean mRecyclerViewIsDragging = false;

    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {

        if (!isEnabled() || canChildScrollUp() || mRefreshing || (!mIsScrollingDown)) {
            Log.d(TAG, "onInterceptTouchEvent: false -- " + " canChildScrollUp() " + canChildScrollUp() + " mRefreshing " + mRefreshing+"  mIsScrollingDown "+mIsScrollingDown);
            return false;
        }
    ...
    private boolean canChildScrollUp() {
        ...
        LinearLayoutManager layoutManager = (LinearLayoutManager) mTarget.getLayoutManager();

        Log.d(TAG, "canChildScrollUp: mIsScrollingDown " + mIsScrollingDown + "  mRecyclerViewIsDragging " + mRecyclerViewIsDragging);
...
//            }
//        } else {
//            boolean canScrollVertically = ViewCompat.canScrollVertically(mTarget, 1);
//            Log.d(TAG, "canChildScrollUp() returned: " + canScrollVertically);
